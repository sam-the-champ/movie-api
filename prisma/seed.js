...
seedMovies().catch((error) => {
    ...
    process.exit(1);
}).finally(async () => {  //no matter if the seeding is successful or not, we want to disconnect from the database
    await prisma.$disconnect();
})



//we are going to import the prisma client from prismaclient, then we are going to create the instanceof prisma.
//the reasom why we are creating a new instance of prisma client instead of just using the one that we had from our config db.js is
//becos,the seed file won't be part of our api. it is going to be a script that we can run whwnever we want to seed our db
//